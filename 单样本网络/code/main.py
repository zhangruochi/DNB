...
def var_filter(dataset, percent=0.8):
    ...
    print("the raw dataset shape is: " + str(dataset.shape))
...
def main():
    ...
    with open("refernce_result.pkl", "wb") as f:
        pickle.dump(reference_result_list,f)
        
            

#-----------------------------SSN--------------------------------------
    # 得到需要检测的单样本
    sample = [time_dataset[name] for time_dataset, name in zip(
        normalize_raw_case_dataset, option_dict["test_sample_name"])]
    # print(sample)
    # exit()
    for index, time_dataset in enumerate(normalize_raw_case_dataset):
        time_dataset[option_dict["test_sample_name"]
                     [i] + "_sample"] = sample[index]
    ...
    for i in range(len(case_dataset_list)):
        ...
        # 对于上述 reference 添加需要测试的新样本
        for cluster_reference in normalize_cluster_list:
            for index, time_cluster_reference in enumerate(cluster_reference):

                time_cluster_reference[option_dict["test_sample_name"]
                                       [i] + "_sample"] = sample[i]

        # 计算SSN
        result_SSN = multiprocessing_clusetr(
            normalize_cluster_list, normalize_raw_case_dataset)

        SSN_result_list.append(result_SSN)

        print("\n")
        print("the {} of time period: \n".format(i))
        print("result: " + str(result) + "\n")

    with open("SSN_result.pkl", "wb") as f:
        pickle.dump(SSN_result_list,f)
